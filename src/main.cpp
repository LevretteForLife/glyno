...
#if defined(NDEBUG)
#error "Glyno cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Glyno
...
{
    RenameThread("glyno-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZGLNMinted()
{
...
void RecalculateZGLNSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGLN supply
...
bool RecalculateGLNSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert GLN to zGLN
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Glyno: We use certain sporks during IBD, so check to see if they are
